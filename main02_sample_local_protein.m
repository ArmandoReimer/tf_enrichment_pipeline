...
for i = 1:numel(nucleus_struct)
    ...
    for j = 1:numel(newVectorFields)
        nucleus_struct(i).(newVectorFields{j}) = NaN(size(reference));
    end
end
...
if segmentNuclei
    ...
    segmentNuclei_main02(yDim, xDim, segment_indices, setFrameArray, setReference, frameReference, nucleusXReference, ...
        ...
        smoothingKernel_px, neighborhoodSize_px, refPath, display_figures, minArea_px, maxArea_px, DropboxFolder, nuclear_mov)
...
for i = 1:size(setFrameArray,1)
    ...
    qc_mat = struct;
    for j = 1:numel(nucleusXVector)
        % get location info
        x_nucleus = round(nucleusXVector(j));
        y_nucleus = round(nucleusYVector(j));
        ...
        if isnan(x_spot) || ~proteinQualityControlVector(j)
            continue
        end
        
        % extract mask
        spot_nc_mask =  nc_ref_frame == nucleusMasterVector(j);
...
%         % in vicinity of spot
        spot_protein_vec(j) = nanmean(protein_frame(int_id==spotRoiFrame));
        spot_mcp_vec(j) = nanmean(mcp_frame(int_id==spotRoiFrame));
        
        % volume protein sampling
        spot_protein_vec_3d(j) = sample_protein_3D(x_spot3D,y_spot3D,z_spot3D,xReferenceMesh,yReferenceMesh,zReferenceMesh,xy_sigma_px,z_sigma_px,protein_stack);
        ...
        % sample snippets
        spot_protein_snips(:,:,j) = sample_snip(x_spot,y_spot,proteinSnippetSize_px,protein_frame,spot_nc_mask);
        spot_mcp_snips(:,:,j) = sample_snip(x_spot,y_spot,proteinSnippetSize_px,mcp_frame,spot_nc_mask);
        ...
        mf_null_protein_vec(j) = nanmean(protein_frame(meanFreeSampleMask));% / voxel_size;
        % Edge sampling
        ...
        [edge_null_x_vec(j), edge_null_y_vec(j), edge_null_nc_vec(j), edge_qc_flag_vec(j),~]...
            ...
            j, minSampleSeparation_px, spot_nc_mask,0);
        ...
        if edge_qc_flag_vec(j) == 0
            ...
            interNuclearDisplacementVector = interNuclearDisplacementMatrix(:,j);
            interNuclearDisplacementVector(j) = Inf;
        ...
        if edge_qc_flag_vec(j) > 0
            ...
            xControl = edge_null_x_vec(j);
            yControl = edge_null_y_vec(j);
            
            null_dist_frame = zeros(size(protein_frame));
            null_dist_frame(yControl,xControl) = 1;
            null_dist_frame = bwdist(null_dist_frame);
            edge_null_protein_vec(j) = nanmean(protein_frame(nc_ref_frame>0&null_dist_frame<roiRadiusSpot_px));% / voxel_size;
            edge_null_mcp_vec(j) = nanmean(mcp_frame(nc_ref_frame>0&null_dist_frame<roiRadiusSpot_px));% / voxel_size;
            
            % take 3D protein sample
            edge_null_protein_vec_3d(j) = sample_protein_3D(xControl,yControl,z_spot3D,xReferenceMesh,yReferenceMesh,zReferenceMesh,xy_sigma_px,z_sigma_px,protein_stack);
            
            % draw snips
            edge_null_protein_snips(:,:,j) = sample_snip(xControl,yControl,proteinSnippetSize_px,protein_frame,nc_ref_frame>0);
            edge_null_mcp_snips(:,:,j) = sample_snip(xControl,yControl,proteinSnippetSize_px,mcp_frame,nc_ref_frame>0);
        ...
        % sample protein
        serial_qc_flag_vec(j) = ~isnan(xControl);
        serial_null_protein_vec(j) = nanmean(protein_frame(nc_ref_frame>0&null_dist_frame<roiRadiusSpot_px));% / voxel_size;
        % record
        serial_null_x_vec(j) = xControl;
        serial_null_y_vec(j) = yControl;
        serial_null_edge_dist_vec(j) = ec;
        % 3D version
        serial_null_protein_vec_3d(j) = sample_protein_3D(xControl,yControl,z_spot3D,xReferenceMesh,yReferenceMesh,zReferenceMesh,xy_sigma_px,z_sigma_px,protein_stack);% / sum(vol_denominator(:)) / voxel_size;
        % check for presence of sister spot
        x_spot_sister = NaN;
        y_spot_sister = NaN;
        if sum(nucleusMasterVector(j)==nucleusMasterVector) == 2
            indices = find(nucleusMasterVector(j)==nucleusMasterVector);
        ...
        qc_mat(numel(nucleusXVector)-j+1).setID = setID;
        qc_mat(numel(nucleusXVector)-j+1).frame = frame;
        qc_mat(numel(nucleusXVector)-j+1).nc_index = nc_lin_index_vec(j);
        qc_mat(numel(nucleusXVector)-j+1).nc_sub_index = nc_sub_index_vec(j);
        qc_mat(numel(nucleusXVector)-j+1).qc_flag = edge_qc_flag_vec(j);
        qc_mat(numel(nucleusXVector)-j+1).xp = x_spot;
        qc_mat(numel(nucleusXVector)-j+1).yp = y_spot;
        qc_mat(numel(nucleusXVector)-j+1).xp_sister = x_spot_sister;
        qc_mat(numel(nucleusXVector)-j+1).yp_sister = y_spot_sister;
        qc_mat(numel(nucleusXVector)-j+1).xc_edge = edge_null_x_vec(j);
        qc_mat(numel(nucleusXVector)-j+1).yc_edge = edge_null_y_vec(j);
        qc_mat(numel(nucleusXVector)-j+1).xc_serial = serial_null_x_vec(j);
        qc_mat(numel(nucleusXVector)-j+1).yc_serial = serial_null_y_vec(j);
        qc_mat(numel(nucleusXVector)-j+1).ParticleID = particle_id_vec(j);
        qc_mat(numel(nucleusXVector)-j+1).serial_qc_flag = serial_qc_flag_vec(j);
        qc_mat(numel(nucleusXVector)-j+1).edge_qc_flag = edge_qc_flag_vec(j);
        sz = neighborhoodSize_px;
        edge_dist_mat = nucleusDistanceFrame;
        edge_dist_mat(~spot_nc_mask&~null_mask) = 0;
        if edge_qc_flag_vec(j) == 2
            sz = max([neighborhoodSize_px,abs(x_nucleus - edge_null_x_vec(j)),abs(y_nucleus - edge_null_y_vec(j))...
        ...
        qc_mat(numel(nucleusXVector)-j+1).x_origin = x_range(1);
        qc_mat(numel(nucleusXVector)-j+1).y_origin = y_range(1);
        qc_mat(numel(nucleusXVector)-j+1).mcp_snip = mcp_frame(y_range,x_range);
        qc_mat(numel(nucleusXVector)-j+1).protein_snip = protein_frame(y_range,x_range);
        qc_mat(numel(nucleusXVector)-j+1).edge_dist_snip = edge_dist_mat(y_range,x_range);
    end
    qualityControlStructure(i).qc_mat = fliplr(qc_mat);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % save snip data
    % initialize struct to store snip data
    snip_data = struct;
    % map data back to nucleus_struct
    for j = 1:numel(nucleusMasterVector)
        nc_index = nc_lin_index_vec(j);
        nc_sub_index = nc_sub_index_vec(j);
        frame = nucleus_struct(nc_index).frames(nc_sub_index);
        for k = 1:numel(newVectorFields)
            vec = eval(newVectorFields{k});
            nucleus_struct(nc_index).(newVectorFields{k})(nc_sub_index) = vec(j);
        end
        % store snips
        for k = 1:numel(newSnippetFields)
            snip = eval([newSnippetFields{k} '(:,:,j)']);
            snip_data.(newSnippetFields{k})(:,:,j) = snip;
        end
    end
    ...
    % report time
    t = round(toc);
    disp([num2str(i) ' of ' num2str(size(setFrameArray,1)) ' frames completed (' num2str(t) ' sec)'])
end
